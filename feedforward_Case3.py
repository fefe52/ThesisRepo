...
def main():
    ...
    def plot_history(history):
        ...
        plt.title('MAE using MLP on HD-sEMG data - study case 3')
        plt.plot(hist['epoch'], hist['mean_absolute_error'],label='Train Error')
        plt.plot(hist['epoch'], hist['val_mean_absolute_error'],label = 'Val Error')
        plt.legend()
        plt.savefig(CWD + '/figures/Case3/HDEMG/MLP/HDEMG_MAE_studycase3.png')

        plt.figure()
        plt.xlabel('Epoch')
        plt.ylabel('Mean Square Error ')
        plt.title('MSE using MLP on HD-sEMG data - study case 3')
        plt.plot(hist['epoch'], hist['mean_squared_error'], label='Train Error')
        plt.plot(hist['epoch'], hist['val_mean_squared_error'], label='Val Error')
        plt.legend()
        plt.savefig(CWD + '/figures/Case3/HDEMG/MLP/HDEMG_MSE_studycase3.png')
        plt.show()

        plt.figure()
        plt.xlabel('Epoch')
        plt.ylabel('Prediction values')
        plt.title('MLP predictions on HD-sEMG training - study case 3') 
        plt.plot(train_target)
        plt.plot(train_targets_pred)
        plt.savefig(CWD + '/figures/Case3/HDEMG/MLP/HDEMG_pred_training_studycase3.png')
        plt.show()
